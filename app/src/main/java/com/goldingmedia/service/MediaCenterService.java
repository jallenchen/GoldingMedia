...
public class MediaCenterService extends Service implements HandlerUtils.OnReceiveMessageListener{
	...
	public static  boolean isMostPlug = false;
	private void register(){
		...
		filter.addAction("android.most.ACTION_MOST_PLUG");
	...
	private final BroadcastReceiver receiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			...
			}else if(Contant.MOST_DET_ACTION.equals(action)) {
			    Log.e("MediaCenterService", "--------------------ACTION_MOST_PLUG:"+intent.getIntExtra("mostplug",1));
			    EventBusCMD cmd = new EventBusCMD(Contant.MsgID.NET_MOST);
			    cmd.setIntValues(intent.getIntExtra("mostplug",1));
			    EventBus.getDefault().post(cmd);
				isMostPlug = cmd.getIntValues() == 0  ? true : false;
				DataSharePreference.saveMostPlugState(mContext,isMostPlug);
			 }
		}
	};    

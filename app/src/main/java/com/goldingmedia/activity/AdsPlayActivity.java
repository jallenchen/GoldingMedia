...
//		try {
//			GDApplication.getmInstance().getIgoldingSysService().exeCmd("speaker");
...
//			mDataList.add(mAdData);
	...
	private void SwitchContent(){// 广告处理器，管理 安全广告 即时广告 定时广告
		...
		if(mPos < mDataList.size()){
			...
			Command.sendCommandString("com.golding.exitadplay");// 广播整个网络，退出广告
			return;
		}
	...
	private synchronized void exitActivity(){
		...
		//mContext.sendBroadcast(intent);
//		try {
//			GDApplication.getmInstance().getIgoldingSysService().exeCmd("headphone");
//		} catch (RemoteException e) {
//			e.printStackTrace();
//		}
